...
function nextTab() {
    currentTabIndex++;
    if (currentTabIndex > 12) { // Reset to the first tab if we've gone past the last one
        currentTabIndex = 0;
    }
...
function switchTabByIndex(tabIndex) {
    const tabs = ['home', 'nowplaying', 'radio', 'songs', 'liked', 'podcasts', 'artists', 'cable', 'updates', 'games', 'media', 'account', 'nowplaying'];
